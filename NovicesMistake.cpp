...
int32_t main()
{
    ...
    while (t--)
    {
        ...
        int num = 0;
        int ptr = 0;
        for(int i = 1;i<=dig * 10000 - 1;i++){
            
            // int count = i / dig, rem = i % dig;
            // int mul = pow(10, dig);
            // while(count){
            //     num = num * mul + n;
            //     count--;
            ...
            // num = (num * pow(10, rem)) + (n/(pow(10, dig - rem)));
            
            num = num * 10 + digits[ptr];
            ptr++;
            ptr = ptr % (digits.size());
            int a = (num - i) / (n - dig);
            int b = a * dig - i;
            
            if(a >= 1 && a <= 10000 && b >= 1 && b <= min(10000LL, a * n) && (dig * a - b) == i && (n * a - b) == num){
                ans.insert({a, b});
            }
        }
