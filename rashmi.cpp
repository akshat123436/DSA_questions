...
int f(int i, int j, int opening,int closing, int n, int m, string &s, vector<vector<vector<int>>> &dp){
    cout << opening << " " << closing << endl;
    if(i>=n){
        if(opening == closing && j == m) return dp[i][opening][j] = 1;
        return 0;
    }
    if(dp[i])
    int ans = 0;
    ans += f(i+1, j, opening+1, closing, n, m, s, dp);
    ans += f(i+1, j, opening, closing + 1, n, m, s, dp);
    if(j<m && s[j] == '('){
        ans += f(i+1, j+1, opening+1, closing, n,m,s,dp);
    }
    else if(j<m && s[j] == ')'){
        ans += f(i+1, j+1, opening, closing+1, n,m,s,dp);
    }
    
    return dp[i][opening][j] = ans;
}
