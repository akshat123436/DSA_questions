...
int32_t main()
{
    ...
    while (t--)
    {
        ...
        int curAns = 0;
        for(int i = 0;i<n;i++){
            if(sum[i] == (i+1)*(i+2) / 2) curAns++;
        }
        for(int i = 0;i<n-1;i+=2){
            int curSum = sum[i];
            int t = 0;
            
            int newSum = curSum - arr[i] + arr[i+1];
            if(newSum == (i+1)*(i+2) / 2){
                first.push_back(1);
            }
            else if(curSum == (i+1)*(i+2) / 2){
                first.push_back(-1);
            }
        ...
        int maxContSum1 = 0, maxContSum2 = 0;
        int s = 0;
        for(auto &a : first){
            s += a;
        ...
        for(auto &a : second){
            ...
            maxContSum2 = max(maxContSum2, s);
        }
        
        cout << curAns + max(maxContSum1, maxContSum2) << endl;
    }
}