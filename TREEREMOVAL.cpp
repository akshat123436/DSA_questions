...
int32_t main()
{
    ...
    while (t--)
    {
        ...
        int lowestWeight = 1e11;
        int lowNode = 0;
        for(int i = 0;i<n;i++){
            cin >> weight[i+1];
            if(weight[i+1] < lowestWeight){
                lowNode = i+1;
                lowestWeight = weight[i+1];
            }
        }
        ...
        while(removedCount < n - 1){
            ...
            for(auto &a : graph[cur.second]){
                // cout << indegree[a] << " " << a << endl;
                if(!indegree[a] || ) continue;
                s.erase({indegree[a], a});
                indegree[a]--;
                s.insert({indegree[a], a});
            }
            removedCount++;
        }
        cout << endl;
